...
internal fun createPetsRepository(
    ...
    return PetsRepositoryImpl(
        ...
        activitiesDataStore =
            createActivitiesDataStore(*activities.toTypedArray()),
    )
}

internal fun createPetsRepository(
    petsDataStore: DataStore<ID, Pet> =
        createPetsDataStore(*createMultiplePets(0).toTypedArray()),
    activitiesDataStore: DataStore<ID, Activity> =
        createActivitiesDataStore(
            *createMultipleActivities("", 0).toTypedArray()
        ),
): PetsRepository {
    return PetsRepositoryImpl(
...
internal fun createPetsDataStore(vararg pets: Pet): DataStore<ID, Pet> {
    ...
    pets.forEach { pet -> store.set(pet.id, pet) }

    return store
}

internal fun createActivitiesDataStore(
    vararg activities: Activity
): DataStore<ID, Activity> {
    val store = MockDataStore<ID, Activity>()

    activities.forEach { activity -> store.set(activity.id, activity) }

    return store
}

internal fun createMultiplePets(amount: Int = 5): List<Pet> {
    return (0..amount).map { index -> createPet(id = "$index") }
}

internal fun createMultipleActivities(
    petId: String,
    amount: Int = 5,
): List<Activity> {
    return (0..amount).map { index -> Activity.EMPTY(petId, "$index") }
}

internal fun createPet(id: String = PET_ID, name: String = ""): Pet {
    return Pet.EMPTY(id = id, name = name)
}

internal fun createActivity(petId: String, id: String = ACTIVITY_ID): Activity {
    return Activity.EMPTY(id = id, petId = petId)
}
