...
sealed class SaveActivityException(message: String) : Exception(message) {
    ...
    class NoSavedPet(id: String) :
        SaveActivityException(NO_SAVED_PET_MESSAGE(id))
    ...
    class DifferentPetId(
        ...
        SaveActivityException(
            DIFFERENT_PET_ID(petId, expectedPetId, activityId)
        )

    companion object {
        private fun NO_SAVED_PET_MESSAGE(id: String) =
            "No pet saved with id {$id} in data store."
